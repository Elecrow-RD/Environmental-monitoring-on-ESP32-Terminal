...
{
    lv_timer_handler();
    lv_label_set_text(ui_qaiNum, "Hello, World!");
    for(int i=0;i<1800;i++){
      lv_img_set_angle(ui_pointer, i);
      delay( 1 );
    }
  ...
  Serial.println(t);
  s = t;
  lv_slider_set_value(ui_Slider2, s, LV_ANIM_ON);
  ...
  //lv_label_set_text(ui_Label2, const char * text);
  if(t> 29.8){
    digitalWrite(19, HIGH);
    fan_Animation(ui_Image2, 0);
    lv_obj_set_style_bg_img_src(ui_Screen1,&ui_img_high_temperature_png , LV_PART_MAIN);
    //delay( 1000 );
    }
    else{
      digitalWrite(19, LOW);
       lv_obj_set_style_bg_img_src(ui_Screen1, &ui_img_normal_temperature_png,LV_PART_MAIN);
      }
  delay( 100 );
}
